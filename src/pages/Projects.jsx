...
export default function Projects() {
  ...
  const fetchData = async () => {
    try {
      ...
      setProjects(
        Array.isArray(projRes.data) ? projRes.data : (projRes.data?.data ?? []),
      );
      setClients(
  ...
  const filteredProjects = useMemo(() => {
    return projects.filter((project) => {
      ...
      const clientName =
        clientNameById.get(project.client_id)?.toLowerCase() || "";
      const search = searchTerm.toLowerCase();

      return projectTitle.includes(search) || clientName.includes(search);
    });
  ...
  <TextField
    fullWidth
    placeholder="Pesquisar por nome do projeto ou parceiro comercial..."
    value={searchTerm}
...
>
      ...
      <Stack direction="row" spacing={2} alignItems="center">
        ...
        <Box>
          ...
          <Typography variant="body2" sx={{ color: '#64748B', fontWeight: 500 }}>
            Configure as diretrizes e o orçamento do novo contrato.
          </Typography>
        </Box>
      </Stack>
    </Box>
  ...
  <DialogContent sx={{ p: 4, bgcolor: '#FFF' }}>
    <Grid container spacing={4}>
      ...
      <Grid item xs={12} md={3.5}>
        <Stack spacing={3}>
          <Typography variant="caption" sx={{ color: '#475569', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px' }}>
      ...
      <Grid item xs={12} md={4.5}>
        <Stack spacing={3}>
          <Typography variant="caption" sx={{ color: '#475569', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px' }}>
            02. Definição de Escopo
          </Typography>
          <TextField 
            fullWidth 
            multiline 
            rows={8} 
            label="Breve Descrição Técnica" 
            placeholder="Quais tecnologias, prazos internos e entregas principais..."
            sx={inputStyle}
  ...
  <DialogActions sx={{ p: 4, bgcolor: '#F8FAFC', borderTop: '1px solid #F1F5F9' }}>
    ...
    >
      {saving ? <CircularProgress size={24} color="inherit" /> : "Efetivar Contrato"}
    </Button>
  </DialogActions>
</Dialog>
